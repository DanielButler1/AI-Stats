...
/// struct for typed errors of method [`video_generation_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum VideoGenerationPostError {
...
pub async fn video_generation_post(configuration: &configuration::Configuration, video_generation_request: models::VideoGenerationRequest) -> Result<models::VideoGenerationResponse, Error<VideoGenerationPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_video_generation_request = video_generation_request;

    let uri_str = format!("{}/video/generation", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<VideoGenerationPostError> = serde_json::from_str(&content).ok();
