...
module AIStatsSdk
  class ResponsesResponseAllOfChoices < ApiModelBase
    attr_accessor :index
    ...
    def self.openapi_types
      {
        :'index' => :'Integer',
        :'message' => :'ResponsesResponseAllOfMessage',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AIStatsSdk::ResponsesResponseAllOfChoices` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AIStatsSdk::ResponsesResponseAllOfChoices`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] finish_reason Value to be assigned
    def finish_reason=(finish_reason)
      if finish_reason.nil?
        fail ArgumentError, 'finish_reason cannot be nil'
      end
