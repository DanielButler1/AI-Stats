...
module AIStatsSdk
  class GatewayChatChoice < ApiModelBase
    attr_accessor :index
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AIStatsSdk::GatewayChatChoice` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AIStatsSdk::GatewayChatChoice`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      finish_reason_validator = EnumAttributeValidator.new('String', ["stop", "length", "tool_calls", "content_filter", "error"])
    ...
    def finish_reason=(finish_reason)
      validator = EnumAttributeValidator.new('String', ["stop", "length", "tool_calls", "content_filter", "error"])
    ...
    def hash
      [index, message, finish_reason, reasoning].hash
    end
