...
import { guardAuth, type GuardErr } from "@/lib/main-endpoints/before/guards";
...
type ModelDataSection = {
    input_types: string[];
    output_types: string[];
};

type ModelGatewaySection = {
    active: boolean;
    endpoints: Endpoint[];
...
type CatalogueModel = {
    ...
    aliases: string[];
    data?: ModelDataSection;
    gateway?: ModelGatewaySection;
};
...
function compareModels(a: CatalogueModel, b: CatalogueModel): number {
    const aPrimary = parseDate(a.release_date) ?? parseDate(a.announcement_date) ?? Number.NEGATIVE_INFINITY;
    const bPrimary = parseDate(b.release_date) ?? parseDate(b.announcement_date) ?? Number.NEGATIVE_INFINITY;
...
async function fetchCatalogue(filter: CatalogueFilters): Promise<CatalogueModel[]> {
    ...
    const { data: modelRows, error: modelError } = await supabase
        .from("data_models")
        .select(
            "model_id, name, release_date, announcement_date, deprecation_date, retirement_date, status, input_types, output_types, organisation:data_organisations!data_models_organisation_id_fkey(organisation_id, name, country_code)"
        );
    if (modelError) {
    ...
    const providerIdsFilter = normalizeStringSet(filter.providerIds);
    let providerQuery = supabase
        .from("data_api_provider_models")
        .select(
            "model_id, api_provider_id, provider_model_slug, endpoint, is_active_gateway, input_modalities, output_modalities, effective_from, effective_to"
        )
        .in("model_id", modelIds);

    if (providerIdsFilter?.length) {
        if (providerIdsFilter.length === 1) {
            providerQuery = providerQuery.eq("api_provider_id", providerIdsFilter[0]!);
        } else {
            providerQuery = providerQuery.in("api_provider_id", providerIdsFilter);
        }
    }
    ...
    const includeEndpoints = normalizeStringSet(filter.includeEndpoints)?.map((value) => value as Endpoint);
    const excludeEndpoints = normalizeStringSet(filter.excludeEndpoints)?.map((value) => value as Endpoint);
    ...
    for (const [modelId, info] of baseModels) {
        ...
        if (!filter.includeRumoured && status === "rumoured") continue;
        if (!filter.includeDeprecated && status === "deprecated") continue;
        if (!filter.includeRetired && status === "retired") continue;
        ...
        if (providerIdsFilter?.length && providerEntries.length === 0) continue;

        const endpoints = Array.from(
            new Set(providerEntries.map((entry) => entry.endpoint))
        ).sort();

        if (includeEndpoints?.length) {
            const hasIncluded = includeEndpoints.some((endpoint) => endpoints.includes(endpoint));
            if (!hasIncluded) continue;
        }

        if (excludeEndpoints?.length) {
            const hasExcluded = excludeEndpoints.some((endpoint) => endpoints.includes(endpoint));
        ...
        const model: CatalogueModel = {
            ...
            announcement_date: info.announcement_date ?? null,
            deprecation_date: info.deprecation_date ?? null,
            retirement_date: info.retirement_date ?? null,
            status: info.status ?? null,
            organisation: info.organisation,
            aliases,
        };

        if (filter.includeDataSection) {
            model.data = {
        ...
        if (filter.includeGatewaySection) {
            model.gateway = {
                active: providerEntries.length > 0,
                endpoints,
...
async function handleModels(req: Request) {
    ...
    const includeEndpoints = parseMultiValue(url.searchParams, "include_endpoints");
    const excludeEndpoints = parseMultiValue(url.searchParams, "exclude_endpoints");
    ...
    const includeRumoured = parseBooleanParam(url.searchParams.get("include_rumoured"), true);
    const includeDeprecated = parseBooleanParam(url.searchParams.get("include_deprecated"), true);
    const includeRetired = parseBooleanParam(url.searchParams.get("include_retired"), true);
    const includeDataSection = parseBooleanParam(url.searchParams.get("include_data"), true);
    const includeGatewaySection = parseBooleanParam(url.searchParams.get("include_gateway"), true);
