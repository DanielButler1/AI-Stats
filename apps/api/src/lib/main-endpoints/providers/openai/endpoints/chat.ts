...
function mapOpenAIToGatewayChat(model: string, json: any): GatewayCompletionsResponse {
...
function createOpenAIStreamResponse(
    ...
    const finalizeResponse = (response: any) => {
        ...
        finalNormalized = mapOpenAIToGatewayChat(opts.model, response);
...
async function execNonStreaming(
    ...
    const normalized = json ? mapOpenAIToGatewayChat(args.model, json) : undefined;
...
export async function exec(args: ProviderExecuteArgs): Promise<AdapterResult> {
    ...
    const modifiedBody: ChatCompletionsRequest = {
        ...
        stream: args.stream ?? sanitizedBody.stream ?? false,
    };
    const requestPayload = mapGatewayToOpenAIResponses(modifiedBody);
    const key = keyInfo.key;

    if (args.stream) {
        return execStreaming(args, key, keyInfo, requestPayload);
    }
    return execNonStreaming(args, key, keyInfo, { ...requestPayload, stream: false });
}
