...
export async function handleSuccessAudit(
    ...
    // Calculate throughput if not already in meta (e.g., for streaming requests)
    if (ctx.meta.throughput_tps === undefined && generationMs > 0) {
        const usage = usageWithMultimodal ?? {};
        const tokensIn = Number(usage.input_text_tokens ?? usage.prompt_tokens ?? usage.input_tokens ?? 0);
        const tokensOut = Number(usage.output_text_tokens ?? usage.completion_tokens ?? usage.output_tokens ?? 0);
        const totalTokens = tokensIn + tokensOut;
        ctx.meta.throughput_tps = totalTokens / (generationMs / 1000);
    }
