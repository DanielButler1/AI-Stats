...
export default function ModelOverview({ model }: ModelOverviewProps) {
	...
	const blended =
		cheapestInput && cheapestOutput
			? (3 * cheapestInput.price + cheapestOutput.price) / 4
			: null;

	const cheapestCachedInput = prices.reduce((min, p) => {
		const price = getPrice(p, "cached_input");
		const provider = getProvider(p);
		return price != null && (min == null || price < min.price)
			? { price, provider, obj: p }
			: min;
		...
		<div className="w-full mx-auto">
			<div className="grid grid-cols-1 gap-6">
				...
				<Pricing
					...
					cheapestCachedInput={cheapestCachedInput}
					blended={blended}
				/>
			</div>
			...
			{/* Links section, now separate from the grid */}
			<div className="mt-4">
